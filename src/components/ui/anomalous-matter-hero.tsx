...
export function GenerativeArtScene() {
  ...
  useEffect(() => {
    ...
    // Get sky color from CSS variable - brighter for visibility
    const getSkyColor = () => {
      if (typeof window !== 'undefined') {
        const root = document.documentElement;
        const sky300 = getComputedStyle(root).getPropertyValue('--sky-300').trim();
        // Convert HSL string to Three.js Color
        // Format: "203, 92%, 53%"
        const [h, s, l] = sky300.split(',').map(v => parseFloat(v.replace('%', '').trim()));
        // Increase lightness for better visibility on black background
        return new THREE.Color().setHSL(h / 360, s / 100, Math.min(l / 100 + 0.2, 0.9));
      }
      // Fallback color (brighter sky blue)
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `                uniform vec3 color;
                ...
                void main() {
                    ...
                    // Increase brightness and add base emission for visibility
                    vec3 baseEmission = color * 0.3;
                    vec3 finalColor = color * diffuse * 1.5 + color * fresnel * 1.2 + baseEmission;
                    
                    // Ensure minimum brightness
                    finalColor = max(finalColor, color * 0.4);
